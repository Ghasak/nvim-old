...
local function myStartifyConfigLauncher()
	...
	if vim.fn.empty(vim.fn.glob(install_path)) > 0 then
		vim.api.nvim_command(
			(
				[[echohl WarningMsg | echomsg "[+] Startify library at :%s is not existed, will be installed after packer sync and compiled ." | echohl None]]
			):format(install_path)
		)
	else
...
local function myQuoteConfigLauncher()
	...
	if vim.fn.empty(vim.fn.glob(install_path)) > 0 then
		vim.api.nvim_command(
			(
				[[echohl WarningMsg | echomsg "[+] Nvim Notify library for my Quote generator at :%s is not existed, will be installed after packer sync and compiled ." | echohl None]]
			):format(install_path)
		)
	else
